...
from ipywidgets import Layout, Label, Text, Checkbox, Button, BoundedIntText, HBox, VBox, Box, \
    FloatText, Dropdown, interactive, IntSlider
...
# )
            ...
            # self.tab = VBox([controls_box, self.fig, download_row])   # plotly approach
...
#   Called from driver module (e.g., pc4*.py) (among other places?)
    ...
    def update_max_frames(self,_b):
        print('update_max_frames: max_frames =',self.max_frames.value)
        # self.i_plot.children[0].max = self.max_frames.value
        self.plot_substrate(self.max_frames.value)
...
#        self.field_index = self.mcds_field.value + 4
        ...
        print('mcds_field_cb: field_index=',self.field_index)
        # self.i_plot.update()
...
#                ygrid = M[1, :].reshape(self.numy, self.numx)
            ...
            if (self.cmap_fixed.value):
                ...
                try:
                    ...
                    x = np.random.randn(frame*200 + 1000)
                    y = np.random.randn(frame*200 + 1000)
                    self.fig = go.FigureWidget(data=go.Histogram2dContour(x=x, y=y, contours=dict(coloring='heatmap')))
    ...
    # assume "frame" is cell frame #, unless Cells is togggled off, then it's the substrate frame #
    # def plot_substrate(self, frame, grid):
    def plot_substrate0(self, frame):
        # global current_idx, axes_max, gFileId, field_index
